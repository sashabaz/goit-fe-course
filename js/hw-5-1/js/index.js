...
const getUsersByGender = (arr, userGender) =>
  arr.filter(users => users.gender === userGender).map(arr => arr.name);
...
const getUsersByFriend = (arr, name) =>
  arr.filter(users => users.friends.includes(name)).map(arr => arr.name);
